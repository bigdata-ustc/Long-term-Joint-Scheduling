...
if __name__ == "__main__":
    ...
    bikeGame = BikeGame(station_num, all_bikes_num, day_mean, max_scheduling_num)
    game_state = bikeGame.get_game_state()
    print("all_bikes_num", sum(bikeGame.bike_flow.graph_flow_list))
    for i in range(7,24):
        award = bikeGame.do_action(-1,-1,0)
        print("time:", i," award", award)
    
    bikeGame.reset_game()
    for i in range(7, 24):
        print("time",i)
        biking_demand = bikeGame.get_biking_demand()
        biking_demand += bikeGame.get_biking_demand(-1)
        biking_num = np.minimum(biking_demand, bikeGame.get_station_bikes_num())
        need_bikes_num = biking_demand - biking_num
        left_bikes_num = bikeGame.get_station_bikes_num() - biking_num
        st = arange(station_num)[left_bikes_num==max(left_bikes_num)][0]
        ed = arange(station_num)[need_bikes_num==max(need_bikes_num)][0]
        num = min(left_bikes_num[st], need_bikes_num[ed])
        print(bikeGame.do_action(st, ed, num))
